...
public class Readertype extends BaseEntity
{
    ...
    /** 可借书数量 */
    @Excel(name = "可借书数量")
    @JsonProperty("CanLendQty")  // 合并在这里
    private Long CanLendQty;

    /** 可借书天数 */
    @Excel(name = "可借书天数")
    @JsonProperty("CanLendDay")  // 合并在这里
    private Long CanLendDay;

    /** 可续借次数 */
    @Excel(name = "可续借次数")
    @JsonProperty("CanContinueTimes") // 合并在这里
    private Long CanContinueTimes;

    /** 罚款率(元/天) */
    @Excel(name = "罚款率(元/天)")
    @JsonProperty("PunishRate")  // 合并在这里
    private BigDecimal PunishRate;

    /** 证书有效期(年) */
    @Excel(name = "证书有效期(年)")
    @JsonProperty("DateValid")   // 合并在这里
